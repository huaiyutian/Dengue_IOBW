  ...
  for (j in 1:50) {
    ...
    for (k in 1:51) {
      ...
      for (ratio in seq(1, 25, 1)) {
        ...
        if((temp < EFD_T0) || (temp > EFD_Tm)){
        EFD =0.0;}
        else{
        EFD =(EFD_c)*temp*(temp-EFD_T0)*sqrt(EFD_Tm-temp);}
        
        // pEA: Probability of mosquito egg-to-adult survival  (quadratic)
        if((temp < pEA_T0) || (temp > pEA_Tm)){
        pEA =0.0; }
        else{
        pEA =(pEA_c)*(temp-pEA_T0)*(temp-pEA_Tm);}
        
        // MDR: Mosquito egg-to-adult development rate (day−1)  (briere)  
        if((temp < MDR_T0) || (temp > MDR_Tm)){
        MDR =0.0;}
        else{
        MDR =(MDR_c)*temp*(temp-MDR_T0)*sqrt(MDR_Tm-temp);}
        
        // a: Biting rate (day−1) (briere)
        if((temp < a_T0) || (temp > a_Tm)){
        a =0.0;}
        else{
        a =(a_c)*temp*(temp-a_T0)*sqrt(a_Tm-temp);}
        
        // pMI: Probability of mosquito infection (briere) 
        if((temp < pMI_T0) || (temp > pMI_Tm)){
        pMI =0.0;}
        else{
        pMI =(pMI_c)*temp*(temp-pMI_T0)*sqrt(pMI_Tm-temp);}
        
        // mu: Adult mosquito lifespan (days)  (inverted_quadratic)
        if((temp < mu_th_T0) || (temp > mu_th_Tm)){
        mu =24; }
        else{
        mu =1/((mu_th_c)*(temp-mu_th_T0)*(temp-mu_th_Tm));}
        
        // PDR: Parasite development rate (day−1) (briere)  
        if((temp < PDR_T0) || (temp > PDR_Tm)){
        PDR =0.0;}
        else{
        PDR =(PDR_c)*temp*(temp-PDR_T0)*sqrt(PDR_Tm-temp);}
        
        // b: Probability of mosquito infectiousness (briere) 
        if((temp < b_T0) || (temp > b_Tm)){
        b =0.0;}
        else{
        b =(b_c)*temp*(temp-b_T0)*sqrt(b_Tm-temp);}
        ...
        double K =alpha*N*exp(-EA*(pow((temp-T29),2))/(kappa*(temp+273.0)*(T29+273.0)));
        ...
        //----------------------------------------------------
        double alpha =(EFD_T29*pEA_T29*MDR_T29*(1.0/mu_T29)-mu_T29)/(EFD_T29*pEA_T29*MDR_T29*(1.0/mu_T29));
        double K_0 =alpha*(ratio*pop)*exp(-EA*(pow((temp-T29),2))/(kappa*(temp+273.0)*(T29+273.0)));
        // ratio: mos vs human
        ...
        covar <- covariate_table(
          t=Cover_trait$time, temp=Cover_trait$temp, ratio=Cover_trait$ratio,
...
write.csv(sim_all,"02_output/01_iobw/02_sim_alb.csv")