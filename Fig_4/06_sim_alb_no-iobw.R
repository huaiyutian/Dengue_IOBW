...
dir.create("02_output/02_no_iobw")
#================================================================================
CaseAll <- read_csv("01_data/01_dengue_mt_14_19.csv")
CoverAll <- read_csv("01_data/04_covar_day_13-19_yes-no_iobw-cesm.csv")

# Select Country
countrys <- unique(CaseAll$country)

# Initial scale
init.cond <- read_csv("01_data/07_init.cond.csv")
  ...
  for (j in 1:50) {
    ...
    for (k in 1:51) {
      ...
      for (ratio in seq(1, 25, 1)) {
        ...
        if((temp_non_iobw < EFD_T0) || (temp_non_iobw > EFD_Tm)){
        EFD =0.0;}
        else{
        EFD =(EFD_c)*temp_non_iobw*(temp_non_iobw-EFD_T0)*sqrt(EFD_Tm-temp_non_iobw);}
        
        // pEA: Probability of mosquito egg-to-adult survival  (quadratic)
        if((temp_non_iobw < pEA_T0) || (temp_non_iobw > pEA_Tm)){
        pEA =0.0; }
        else{
        pEA =(pEA_c)*(temp_non_iobw-pEA_T0)*(temp_non_iobw-pEA_Tm);}
        
        // MDR: Mosquito egg-to-adult development rate (day−1)  (briere)  
        if((temp_non_iobw < MDR_T0) || (temp_non_iobw > MDR_Tm)){
        MDR =0.0;}
        else{
        MDR =(MDR_c)*temp_non_iobw*(temp_non_iobw-MDR_T0)*sqrt(MDR_Tm-temp_non_iobw);}
        
        // a: Biting rate (day−1) (briere)
        if((temp_non_iobw < a_T0) || (temp_non_iobw > a_Tm)){
        a =0.0;}
        else{
        a =(a_c)*temp_non_iobw*(temp_non_iobw-a_T0)*sqrt(a_Tm-temp_non_iobw);}
        
        // pMI: Probability of mosquito infection (briere) 
        if((temp_non_iobw < pMI_T0) || (temp_non_iobw > pMI_Tm)){
        pMI =0.0;}
        else{
        pMI =(pMI_c)*temp_non_iobw*(temp_non_iobw-pMI_T0)*sqrt(pMI_Tm-temp_non_iobw);}
        
        // mu: Adult mosquito lifespan (days)  (inverted_quadratic)
        if((temp_non_iobw < mu_th_T0) || (temp_non_iobw > mu_th_Tm)){
        mu =24; }
        else{
        mu =1/((mu_th_c)*(temp_non_iobw-mu_th_T0)*(temp_non_iobw-mu_th_Tm));}
        
        // PDR: Parasite development rate (day−1) (briere)  
        if((temp_non_iobw < PDR_T0) || (temp_non_iobw > PDR_Tm)){
        PDR =0.0;}
        else{
        PDR =(PDR_c)*temp_non_iobw*(temp_non_iobw-PDR_T0)*sqrt(PDR_Tm-temp_non_iobw);}
        
        // b: Probability of mosquito infectiousness (briere) 
        if((temp_non_iobw < b_T0) || (temp_non_iobw > b_Tm)){
        b =0.0;}
        else{
        b =(b_c)*temp_non_iobw*(temp_non_iobw-b_T0)*sqrt(b_Tm-temp_non_iobw);}
        ...
        double K =alpha*N*exp(-EA*(pow((temp_non_iobw-T29),2))/(kappa*(temp_non_iobw+273.0)*(T29+273.0)));
        ...
        //----------------------------------------------------
        double alpha =(EFD_T29*pEA_T29*MDR_T29*(1.0/mu_T29)-mu_T29)/(EFD_T29*pEA_T29*MDR_T29*(1.0/mu_T29));
        double K_0 =alpha*(ratio*pop)*exp(-EA*(pow((temp_non_iobw-T29),2))/(kappa*(temp_non_iobw+273.0)*(T29+273.0)));
        // ratio: mos vs human
        ...
        covar <- covariate_table(
          t=Cover_trait$time, temp_non_iobw=Cover_trait$temp_non_iobw, ratio=Cover_trait$ratio,
...
write.csv(sim_all,"02_output/02_no_iobw/02_sim_alb_no_iobw.csv")