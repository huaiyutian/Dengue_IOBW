...
dir.create("02_output/04_sim-alb_without-iobw")

#================================================================================
CaseAll <- read_csv("01_data/01_dengue_mt_14_19.csv")
CoverAll <- read_csv("01_data/03_covar_day_13-19_20ens_without.csv")

# Select Country
countrys <- unique(CaseAll$country)

# Initial scale
init.cond <- read_csv("01_data/04_init.cond.csv")
# Thermal response parameters
trait_posterior <- read_csv("01_data/06_alb_traits.csv")

all_start_time <- Sys.time()
target_country <-c(1:24)
target_IC <- c(1:50)
target_ID <-c(1:51)
target_ratio <- c(1:25)

for (i in target_country) {
  sim_all <- c()
  ...
  for (j in target_IC) {
    ...
    for (k in target_ID) {
      ...
      for (ratio in target_ratio) {
        ...
        #================================================================================
        seic_step <- Csnippet("
        //=========================Ae. albopictus vector parameters function==============================
        // Parameter values are defined as follows:
        double a,b,EFD,PDR,pMI,pEA,MDR,mu;
        
        // EFD: Eggs laid per female per day (briere)  
        if((temp_without_iobw < EFD_T0) || (temp_without_iobw > EFD_Tm)){
        EFD =0.0;}
        else{
        EFD =(EFD_c)*temp_without_iobw*(temp_without_iobw-EFD_T0)*sqrt(EFD_Tm-temp_without_iobw);}
        
        // pEA: Probability of mosquito egg-to-adult survival  (quadratic)
        if((temp_without_iobw < pEA_T0) || (temp_without_iobw > pEA_Tm)){
        pEA =0.0; }
        else{
        pEA =(pEA_c)*(temp_without_iobw-pEA_T0)*(temp_without_iobw-pEA_Tm);}
        
        // MDR: Mosquito egg-to-adult development rate (day−1)  (briere)  
        if((temp_without_iobw < MDR_T0) || (temp_without_iobw > MDR_Tm)){
        MDR =0.0;}
        else{
        MDR =(MDR_c)*temp_without_iobw*(temp_without_iobw-MDR_T0)*sqrt(MDR_Tm-temp_without_iobw);}
        
        // a: Biting rate (day−1) (briere)
        if((temp_without_iobw < a_T0) || (temp_without_iobw > a_Tm)){
        a =0.0;}
        else{
        a =(a_c)*temp_without_iobw*(temp_without_iobw-a_T0)*sqrt(a_Tm-temp_without_iobw);}
        
        // pMI: Probability of mosquito infection (briere) 
        if((temp_without_iobw < pMI_T0) || (temp_without_iobw > pMI_Tm)){
        pMI =0.0;}
        else{
        pMI =(pMI_c)*temp_without_iobw*(temp_without_iobw-pMI_T0)*sqrt(pMI_Tm-temp_without_iobw);}
        
        // mu: Adult mosquito lifespan (days)  (inverted_quadratic)
        if((temp_without_iobw < mu_th_T0) || (temp_without_iobw > mu_th_Tm)){
        mu =24; }
        else{
        mu =1/((mu_th_c)*(temp_without_iobw-mu_th_T0)*(temp_without_iobw-mu_th_Tm));}
        
        // PDR: Parasite development rate (day−1) (briere)  
        if((temp_without_iobw < PDR_T0) || (temp_without_iobw > PDR_Tm)){
        PDR =0.0;}
        else{
        PDR =(PDR_c)*temp_without_iobw*(temp_without_iobw-PDR_T0)*sqrt(PDR_Tm-temp_without_iobw);}
        
        // b: Probability of mosquito infectiousness (briere) 
        if((temp_without_iobw < b_T0) || (temp_without_iobw > b_Tm)){
        b =0.0;}
        else{
        b =(b_c)*temp_without_iobw*(temp_without_iobw-b_T0)*sqrt(b_Tm-temp_without_iobw);}
        ...
        double K =alpha*N*exp(-EA*(pow((temp_without_iobw-T29),2))/(kappa*(temp_without_iobw+273.0)*(T29+273.0)));
        ...
        //----------------------------------------------------
        double alpha =(EFD_T29*pEA_T29*MDR_T29*(1.0/mu_T29)-mu_T29)/(EFD_T29*pEA_T29*MDR_T29*(1.0/mu_T29));
        double K_0 =alpha*(ratio*pop)*exp(-EA*(pow((temp_without_iobw-T29),2))/(kappa*(temp_without_iobw+273.0)*(T29+273.0)));
        // ratio: mos vs human
        ...
        covar <- covariate_table(
          t=Cover_trait$time, temp_without_iobw=Cover_trait$temp_without_iobw, ratio=Cover_trait$ratio,
  ...
  write.csv(sim_all, paste0("02_output/04_sim-alb_without-iobw/", i, "_", countrys[i], ".csv"))

  end_time <- Sys.time()
  execution_time <- difftime(end_time, start_time, units="hours")
  cat("Country", i, "completed at", format(end_time, format = "%Y-%m-%d %H:%M:%S"), "\n")
  cat("Total excution time: ", execution_time, "hours\n" )
}

all_end_time <- Sys.time()
all_execution_time <- difftime(all_end_time, all_start_time, units="hours")
cat("All excution time: ", all_execution_time, "hours\n" )
