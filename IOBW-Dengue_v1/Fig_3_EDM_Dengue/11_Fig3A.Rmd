...
for (i in 1:length(climate_vars)) {
  ...
  #Bonferroni method
  t.p_bonferroni<-p.adjust(t.p,method="bonferroni")
  t.p_sea_bonferroni <-t.p_bonferroni[1:96]
  t.p_ebi_bonferroni <-t.p_bonferroni[97:192]
  t.p_sea_df <- cbind(data.frame(t.p_sea_bonferroni),ccm_obs$climate)
  t.p_ebi_df <- cbind(data.frame(t.p_ebi_bonferroni),ccm_obs$climate)

  ccm_obs$t.p_sea_bonferroni <-t.p_sea_bonferroni
  ccm_obs$t.p_ebi_bonferroni <-t.p_ebi_bonferroni
  ccm_obs$sign_sea_bonferroni <- ifelse(t.p_sea_bonferroni < 0.05 , 1, 0)
  ccm_obs$sign_ebi_bonferroni <- ifelse(t.p_ebi_bonferroni < 0.05 , 1, 0)
...
for (i in 1:length(climate_vars)) {
  #
  t.p_sea <- t.p_sea_df$t.p_sea_bonferroni[t.p_sea_df$"ccm_obs$climate" == climate_vars[i]]
  t.p_ebi <- t.p_ebi_df$t.p_ebi_bonferroni[t.p_ebi_df$"ccm_obs$climate" == climate_vars[i]]
...
plot_sea <- ggplot(ccm_sea_max_95ci, aes(x = q500, y = country, group=climate, color = climate)) +
  ...
  geom_point(data =ccm_obs, aes(x = rho, y = country, shape=factor(sign_sea_bonferroni)), color = "black", size = 2, stroke = 1) + 
...
plot_ebi <- ggplot(ccm_ebi_max_95ci, aes(x = q500, y = country, group=climate, color = climate)) +
  geom_point(size = 2, stroke = 1)+
  geom_errorbar(aes(xmin = q025, xmax = q975), width = 0.2) +
  geom_point(data =ccm_obs, aes(x = rho, y = country, shape=factor(sign_ebi_bonferroni)), color = "black", size = 2, stroke = 1) + 
  facet_wrap(~climate, scales = "free_x", nrow = 1, labeller = labeller(climate = 
...
ggsave("02_output/01_Fig3A.pdf", plot_all, width = 18, height = 6) 
